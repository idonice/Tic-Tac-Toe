...
const AvatarCarousel = ({ avatars, avatarSubmitHandler }) => {
    ...
    const avatarHandler = (avatarIndex) => {
        setChosenAvatar(avatarIndex)
        avatarSubmitHandler(avatarIndex)
    }
    const nextAvatar = () => {
            ...
            <div className="avatar-carousel">
                {avatars.slice(currentIndex, currentIndex + 3).map((avatar, index) => (
                    <div key={index} className="avatar-slide">
                        <div className="avatar-image" onClick={() => { avatarHandler(index) }} style={{ backgroundImage: `url(${avatar})`, border: `${index == chosenAvatar ? '2px solid #fff' : ''}` }}></div>
                    </div>
                ))}
            </div>
